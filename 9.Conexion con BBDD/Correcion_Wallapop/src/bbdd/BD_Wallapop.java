...
public class BD_Wallapop extends BD_Conector {
	...
	public Usuario buscarUsu(String email) throws ErrorBaseDatos {
		String cadenaSQL = "SELECT * FROM usuarios WHERE mail = '" + email + "'"; //Hace la consulta 
		Usuario u = null;
		try {
			this.abrir();
			s = c.createStatement(); //Abre 
			reg = s.executeQuery(cadenaSQL);

			if (reg.next()) { //Coge el dato
				//Monta un usuario
				u = new Usuario(reg.getString(1), reg.getString(2), reg.getString(3), reg.getDouble(4));
				//Otro orden no es valido 
				}
			s.close();
			this.cerrar();
			return u;
			...
			this.cerrar();
			throw new ErrorBaseDatos("No se puede realizar la consulta");
		}
	}
	...
	public ArrayList<Anuncio> buscarAnuncio(String categoria, double p1, double p2) throws ErrorBaseDatos {
	...
	public int venderAnuncio(Anuncio a) throws ErrorBaseDatos {
		...
		try {
			this.abrir();
			s = c.createStatement(); //Crea un statement
