...
public class Principal {
	...
	public static void main(String[] args) {
		...
		do {
			...
			} while (!email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\\.[a-z]{2,}$")); //Valida el correo 

			try {
				u = bd.buscarUsu(email); //Busca el usuario por el email
		...
		do {
			...
			sc.nextLine();
			try {
				anuncios = bd.buscarAnuncio(categoria, p1, p2);
			} catch (ErrorBaseDatos e) {
				System.out.println(e.getMessage());
			}
			if (anuncios.isEmpty()) {
